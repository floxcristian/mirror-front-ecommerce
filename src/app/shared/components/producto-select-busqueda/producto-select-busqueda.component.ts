...
export class ProductoSelectBusquedaComponent implements OnInit, OnChanges, OnDestroy {
  ...
  async ngOnChanges(changes: SimpleChanges): Promise<void> {
    if (changes['nuevosArticulos'] && this.nuevosArticulos && this.nuevosArticulos instanceof Array) {
      if (this.hayCambios()) {
        if (this.nuevosArticulos.length === 0) {
          this.articulos = [];
        } else if (Object.keys(this.nuevosArticulos).length > 1) { // Vienen todos los atributos
          const search = this.nuevosArticulos.map(x => x.sku).join(',');
          this.articuloInput$.next(search);
          this.articulos = [...this.nuevosArticulos];
        } else { // Vienen solo los sku
          const search = this.nuevosArticulos.map(x => x.sku).join(',');
          this.articuloInput$.next(search);
          const response = await (this.logisticService.articulos(search).toPromise());
          if(response){
  ...
  hayCambios(): boolean {
    if (!this.articulos && this.nuevosArticulos && this.nuevosArticulos.length > 0) {
      return true;
    }
  ...
  buscarArticulo() {
    this.articulosSelect$ = concat(
        ...
        this.articuloInput$.pipe(
          ...
          switchMap(search => this.logisticService.articulos(search).pipe(
            map(r => r.data),
