...
export class AgregarListaProductosUnitariaModalComponent implements OnInit {
    ...
    getListas() {
        this.clientsService.getListaArticulosFavoritos(this.usuario.rut || '0').subscribe(
            (resp: ResponseApi) => {
    ...
    onFiltrosCambiados(articulo: any) {
        if (!this.skus.map(e => e.sku).includes(articulo.sku)) {
            ...
            this.toastr.error(`Artículo SKU: ${articulo.sku} ya se encuentra agregado.`);
        }
    }
    ...
    guardar() {
        ...
        if (!this.creandoLista && !this.seleccionandoLista) {
            return this.toastr.error('Debe crear una lista o seleccionar una existente.');
        }
        ...
        this.guardando = true;
        this.clientsService.setArticulosFavoritosUnitario(request, idLista).subscribe(
            (resp: ResponseApi) => {
                if (!resp.error) {
