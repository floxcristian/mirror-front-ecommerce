...
export class TrackingStepsComponent implements OnInit {
  ...
  async buscarDespachOV(){
    this.estado_envio={};
    let consulta:any=await this.logisticservice.obtieneEstadoOV(this.OV).toPromise();
    let estado_envio:any=[];

    if(consulta.data.length>0){
    if(consulta.data[0].CodigoPrestador=='CORREOSCHI') estado_envio= consulta.data.filter((r:any)=>r.CodigoEstado=='4');
  ...
  buscarRetirOV(){
    let estado_envio=this.OVEstados.filter(r=>r.CodigoSeguimiento==30|| r.CodigoSeguimiento==3 || r.CodigoSeguimiento==31 );
    if(estado_envio.length>0){
  ...
  async informarEstados(){
    ...
    this.OVEstados.forEach(estado=>{
      if(estado.EstadoSegPanel=="CREADO" || estado.EstadoSegPanel=="ORIGEN"){
        ...
        this.Informe_estado[2]=estado;
      }else if(estado.EstadoSegPanel=="RECIBIDO" || estado.EstadoSegPanel=="N/A"){
