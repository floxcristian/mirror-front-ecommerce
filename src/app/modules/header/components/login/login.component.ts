...
export class LoginComponent implements OnInit {
    ...
    entrar() {
        ...
        this.loginService.iniciarSesion(this.form.value).subscribe((r: any) => {
            ...
            if (r.status === 'OK') {
                ...
                if (this.router.url.split('?')[0] != '/carro-compra/confirmar-orden-oc') {
                    console.log('entró')
                    sub = this.cart.items$.subscribe(resp => {
                        ...
                        if (this.router.url == '/sitio/iniciar-sesion' && this.ruta) {
                            resp.length > 0 ? this.router.navigate(['/carro-compra', 'resumen']) : this.router.navigate(this.ruta);

                        } else if (this.router.url != '/sitio/iniciar-sesion' && this.ruta)
                            resp.length > 0 ? this.router.navigate(['/carro-compra', 'resumen']) : (r.data.user_role === 'supervisor' || r.data.user_role === 'comprador') ? this.router.navigate(['/inicio']).then(() => {
                ...
                if (this.router.url.split('?')[0] == '/carro-compra/confirmar-orden-oc' && this.ruta) {
                    console.log('entro', this.ruta, queryParams)
                    // this.router.navigate(this.ruta, { queryParams });
                }
            } else {
            ...
            // this.toastr.error(`Ha ocurrido un error en la conexion`);
