...
import { BuscadorService, BusquedaData } from '../../../../../../shared/services/buscador.service';
...
export class MobileBarraBusquedaComponent implements OnInit {
  ...
  private buscadorExternoRef: Subscription = null;
  ...
  async ngOnInit() {
    ...
    this.buscadorExternoRef = this.buscadorService.buscadorExterno$.subscribe((data: BusquedaData) => {
    ...
    if (url.root.children.primary.segments.length >= 2) {
      if (url.root.children.primary.segments[0].path === 'inicio' && url.root.children.primary.segments[1].path === 'productos') {
    ...
    if (!isVacio(url.queryParams)) {
      if (!isVacio(url.queryParams.chassis)) {
        const f = this.flota.find(e => e.vehiculo.chasis === url.queryParams.chassis);
        const b = this.busquedas.find(e => e.vehiculo.chasis === url.queryParams.chassis);
    ...
    if (!isVacio(texto.trim())) {
      ...
      this.flotaFiltrada = this.flota.filter(
        f =>
          ...
          f.vehiculo.anio.toString().toLowerCase().includes(texto.toLowerCase())
      );
    } else {
  ...
  buscarSelect() {
    if (isVacio(this.textToSearch) && (isVacio(this.filtro) || isVacio(this.marcaModeloAnio))) {
      return;
    }
  ...
  buscar() {
    if (isVacio(this.textToSearch.trim()) && isVacio(this.filtro) && isVacio(this.marcaModeloAnio)) {
      this.toastr.info('Debe ingresar un texto para buscar o seleccionar un filtro.', 'Información');
      return;
    }
  ...
  async getDataDropDown() {
    this.cargando = true;
    const busquedas: ResponseApi = await this.clientsService.getBusquedasVin(this.usuario.rut).toPromise();
    const flota: ResponseApi = await this.clientsService.getFlota(this.usuario.rut).toPromise();
    ...
    if (!isVacio(this.marcaModeloAnio)) {
      ...
      if (isVacio(this.filtro)) {
        ...
        if (!isVacio(this.filtro.alias)) {
          const alias = this.filtro.alias.charAt(0).toUpperCase() + this.filtro.alias.slice(1);
  ...
  agregarVinFlota(busqueda: Flota) {
    ...
    const bsModalRef: BsModalRef = this.modalService.show(AddFlotaModalComponent, { initialState, class: 'mx-md-auto' });
    bsModalRef.content.event.subscribe(async res => {
      if (res !== '') {
        ...
        const respuesta: any = await this.clientsService.setFlota(request).toPromise();
        if (!respuesta.error) {
  ...
  eliminarVinBusqueda(busqueda: Flota) {
    ...
    const bsModalRef: BsModalRef = this.modalService.show(ModalComponent, { initialState, class: 'mx-md-auto' });
    bsModalRef.content.event.subscribe(async res => {
      if (res) {
  ...
  armaQueryParams(queryParams) {
    if (!isVacio(this.filtro)) {
      queryParams = { ...queryParams, ...{ chassis: this.filtro.vehiculo.chasis } };
    }
    if (!isVacio(this.marcaModeloAnio)) {
      queryParams = { ...queryParams, ...{ marca: this.marcaModeloAnio.marca } };
      queryParams = { ...queryParams, ...{ modelo: this.marcaModeloAnio.modelo } };
