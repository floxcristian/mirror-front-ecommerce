...
export class MobileFiltrosComponent implements OnInit {
    ...
    ngOnInit() {
        this.catalogoService.getFiltroAnios().subscribe(
            (resp: ResponseApi) => {
                this.anios = resp.data.map(e => e.anio);
            }
        );
    }
    ...
    buscarChassis() {
        if (isVacio(this.chasisPatente)) {
            ...
            this.clientsService.getVehiculo(this.chasisPatente).subscribe(
                (resp: ResponseApi) => {
    ...
    seleccionaAnio() {
        if (this.anio !== '') {
            this.marca = '';
            this.modelo = '';
            this.catalogoService.getFiltroMarcas(this.anio).subscribe(
                (resp: ResponseApi) => {
                    this.marcas = resp.data.map(e => e.marca);
                }
            );
        } else {
            this.marca = '';
    ...
    seleccionaMarca() {
        if (this.marca !== '') {
            this.modelo = '';
            this.catalogoService.getFiltroModelos(this.anio, this.marca).subscribe(
                (resp: ResponseApi) => {
                    this.modelos = resp.data.map(e => e.modelo);
                }
            );
        } else {
            this.modelo = '';
