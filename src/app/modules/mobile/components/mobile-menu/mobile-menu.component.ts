...
import { TiendaLocation, GeoLocation } from '../../../../shared/interfaces/geo-location';
...
export class MobileMenuComponent implements OnDestroy, OnInit {
    ...
    ngOnInit() {
        ...
        this.root.getPreferenciasCliente().then(
            (preferencias) => {
                this.direccion = preferencias.direccionDespacho;
            }
        );

        this.despachoCliente = this.logisticsService.direccionCliente$.subscribe((r) => {
            this.direccion = r;
        });
    }
            ...
            for (const segundaCategoria of primeraCategoria.children) {
                    ...
                    items: [
                        {
                            ...
                            url: ['/', 'inicio', 'productos', 'todos', 'categoria', this.root.replaceSlash(primeraCategoria.url), this.root.replaceSlash(segundaCategoria.url)],
                            items: ''
                        }
                    ]
                };
                ...
                for (const terceraCategoria of segundaCategoria.children) {
                    const dataLineas = {
                        ...
                        url: ['/', 'inicio', 'productos', 'todos', 'categoria', this.root.replaceSlash(primeraCategoria.url), this.root.replaceSlash(segundaCategoria.url), this.root.replaceSlash(terceraCategoria.url)],
                    };
    ...
    modificarDireccionDespacho() {
        const bsModalRef: BsModalRef = this.modalService.show(DireccionDespachoComponent);
