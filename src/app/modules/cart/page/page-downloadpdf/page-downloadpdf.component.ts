...
export class PageDownloadpdfComponent implements OnInit {
  ...
  downloadOvPdf() {
    ...
    const url = environment.apiImplementosCarro + 'documentos/documentoOVPDF/' + btoa(`@${this.numero}@`);
  ...
  downloadOcPdf() {
    ...
    this.http.get(url, { headers, responseType: 'blob' }).subscribe((response: any) => {
      ...
      this.pdfBase64 = this.sanitizer.bypassSecurityTrustResourceUrl(window.URL.createObjectURL(file3));
    });
  }
  ...
  downloadFacturaPdf() {
    ...
    const url = environment.apiImplementosCarro + 'documentos/documentoClientePDF/' + codigo;
    this.http.get(url, { headers, responseType: 'text' }).subscribe(response => {
      response = response + '#toolbar=1&statusbar=1&navpanes=1';
      this.pdfBase64 = this.sanitizer.bypassSecurityTrustResourceUrl(response);
    });
