...
export class PageOmniCartPaymentMethodComponent implements OnInit {
    ...
    async loadData() {
        let consulta: any = await this.cartService.getCarroOmniChannel(this.id).toPromise();
    ...
    async PaymentWebpay() {
        ...
        let consulta: any = await this.paymentService.createTransBankTransaccion(params);
        if (consulta) {
    ...
    async showRejectedMsg(query: any) {
        ...
        if (query.status === 'rejected') {
            ...
            this.alertCart = {
                pagoValidado: false,
                detalleMensaje: await this.paymentService.getPaymentErrorDetail(query.codRejected),
    ...
    openModal() {
        this.modalRef = this.modalService.show(this.content, { backdrop: 'static', keyboard: false });
    }
}
